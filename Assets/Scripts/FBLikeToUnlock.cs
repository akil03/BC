...
public class FBLikeToUnlock : MonoBehaviour
{
	...
	public IEnumerator CheckIfHaveToEnableOrDisableAds ()
	{
		while (true) {
			...
			if (timeDifference > 3600) {
				//AdNetworks.instance.gameObject.SetActive (true);
			} else {
				//AdNetworks.instance.gameObject.SetActive (false);
			}
	...
	public IEnumerator LikePageRoutine ()
	{
		if (!Liked ()) {
			...
			yield return new WaitForSeconds (2);
			MobileNativeMessage msg = new MobileNativeMessage ("Thanks!", "Thanks for liking our page! Enjoy the game without ads for one hour!");
		}
